...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <h1 class="text-3xl font-bold text-center mb-8 text-gray-900">{{ __('Friend List') }}</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        @forelse ($friends as $f)
            <div class="bg-white rounded-lg shadow-md p-6 flex flex-col items-center text-center hover:shadow-lg transition-shadow duration-200">
                ...
                <div class="text-sm text-gray-500 mb-4">
                    {{ __('Went out together') }} <span class="font-semibold text-gray-700">{{ $f->times_together ?? 0 }}</span> {{ __('times') }}
                </div>
        ...
        @empty
            <div class="col-span-full text-center py-12">
                ...
                <p class="text-gray-500 text-lg">{{ __('No friends found yet.') }}</p>
